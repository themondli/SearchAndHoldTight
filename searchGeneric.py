...
from searchExample import shortestDistance
...
class BFSSearcher(Displayable):
    ...
    def __init__(self, problem):
        ...
        self.add_to_frontier(Path(problem.start_node()))
        self.visited = set()
        self.visited.add(problem.start_node())
        super().__init__()
    ...
    def search(self):
        while not self.empty_frontier():
            self.path = self.frontier.popleft() #pop from queue
            self.num_expanded += 1  
            ...
            if self.problem.is_goal(self.path.end()):    # solution found
                ...
                self.display(1, f"Solution: {self.path} (cost: {self.path.cost})\n",
                    self.num_expanded, "paths have been expanded and",
                            len(self.frontier), "paths remain in the frontier")
                return self.path #solution returned
            else:
                self.display(1,f"Expanding: {self.path} (cost: {self.path.cost})")
                neighs = self.problem.neighbors(self.path.end())
                self.display(1,f"Expanding: {self.path} with neighbors {neighs}")
                for arc in neighs:
                    if arc.to_node not in self.visited:
                        self.visited.add(arc.to_node)
                        self.add_to_frontier(Path(self.path, arc))
                self.display(1, f"New frontier: {[p.end() for p in self.frontier]}")
...
def test(SearchClass, problem):
    ...
    searcher = SearchClass(problem)
    path = searcher.search()
    print("Path found:",path)

if __name__ == "__main__":
    test(AStarSearcher, searchExample.simp_delivery_graph)
    test(BFSSearcher, searchExample.simp_delivery_graph)
    test(Searcher, searchExample.simp_delivery_graph)

    test(AStarSearcher, searchExample.cyclic_simp_delivery_graph)
    test(BFSSearcher, searchExample.cyclic_simp_delivery_graph)
    test(Searcher, searchExample.cyclic_simp_delivery_graph)

    test(AStarSearcher, searchExample.shortestDistance)
    test(BFSSearcher, searchExample.shortestDistance)
    test(Searcher, searchExample.shortestDistance)
...
# example queries:
    ...
    searcher5 = BFSSearcher(searchExample.simp_delivery_graph) # BFS
    searcher5.search()  # find first path
    searcher6 = BFSSearcher(searchExample.cyclic_simp_delivery_graph) # BFS
    searcher6.search()  #find first path 

    searcher7 = AStarSearcher(searchExample.shortestDistance) # AStar
    searcher7.search()  # find first path
    searcher8 = BFSSearcher(searchExample.shortestDistance) # BFS
    searcher8.search()  #find first path
    searcher9 = Searcher(searchExample.shortestDistance) # DFS
    searcher9.search()  # find first path
